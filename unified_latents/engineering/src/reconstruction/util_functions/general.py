...
def get_parent_folder():
    ...
    # Use absolute path for reliability in mirrored structure
    parent_folder = "/home/mithunmanivannan/src/reconstruction/Data/"
...
def get_collection(database_params_file=None):
    ...
    if database_params_file is None:
        # Default to the mirrored config
        database_params_file = "/home/mithunmanivannan/src/reconstruction/database_params.json"
    
    database_params = json.load(open(database_params_file, 'r'))
    
    if database_params.get('host') == 'PTBXL':
        from load_functions.ptbxl_collection import PTBXLCollectionProxy
        return PTBXLCollectionProxy(
            root_dir=database_params['root_dir'],
            csv_file=database_params['csv_file'],
            target_fs=database_params.get('target_fs', 100)
        )
