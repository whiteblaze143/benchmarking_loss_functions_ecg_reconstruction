...
class MIMICDataset(Dataset):
    ...
    def __init__(
        ...
        assert period in ["train", "test", "val"], "period must be train, val or test."
        if period == "train":
    ...
    def read_data(self, filepath):
        ...
        if self.period == "train":
            samples = np.load(os.path.join(filepath, "train_data_20000.npy"))
        elif self.period == "test":
            samples = np.load(os.path.join(filepath, "test_data_2000.npy"))
