...
pending → running → completed
                 ...
                 ↘ failed_transient → pending (after delay) [bounded allowlist only]
...
## Transient CUDA/driver handling

The scheduler separately recognizes a narrow allowlist of infrastructure-like
CUDA faults, including the explicit NVLink/peer-memory hardware-error message.
These become `failed_transient`, wait for `transient_retry.delay`, and retry up
to `transient_retry.max_attempts` total attempts. Device-side assertions,
illegal memory accesses, generic nonzero exits, and arbitrary tracebacks are
not automatically retried because they may be deterministic program/data
failures. A repeated allowlisted fault becomes `stuck`; the queue never retries
it indefinitely.
...
- **Narrow transient retry** — only allowlisted GPU/driver faults retry; deterministic CUDA failures stay visible
