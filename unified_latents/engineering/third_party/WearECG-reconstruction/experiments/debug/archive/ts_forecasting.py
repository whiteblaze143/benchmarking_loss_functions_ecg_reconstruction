...
data_grouper = MultivariateGrouper(
...
class Args_Example:
    def __init__(self) -> None:
        ...
        self.save_dir = "./experiments/forecasting_exp"
        self.gpu = 0
...
trainer = Trainer(
    config=configs, args=args, model=model, dataloader={"dataloader": dataloader}
)
...
test_dataloader = torch.utils.data.DataLoader(
    test_dataset,
    batch_size=test_scaled.shape[0],
    shuffle=False,
    num_workers=0,
    pin_memory=True,
    sampler=None,
...
sample = scaler.inverse_transform(
    unnormalize_to_zero_to_one(sample.reshape(-1, feat_num))
).reshape(test_scaled.shape)
...
for idx in range(5):
    ...
    plt.plot(
        ...
        real[0, : (seq_length - pred_length), idx],
        color="c",
        linestyle="solid",
        label="History",
    )
