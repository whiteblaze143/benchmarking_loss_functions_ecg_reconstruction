...
| Emory-MUSE | 12SL diagnosis-code multilabel classification | 974,172 unique `FileName` rows join to 998,844 metadata rows; 968,680 rows have one or more 12SL codes and 5,492 are explicitly ineligible because `codes` is empty; 349,547 patients; 180-code task vocabulary | Patient-disjoint hash split using `BDSPPatientID` | Fit vocabulary/support rules on training patients only. Keep raw codes, explicit exclusions, and dictionary text in provenance. |
