...
def evaluate_reconstruction(
    ...
    *,
    ...
    with torch.no_grad():
        for x, y, _ in tqdm(val_loader, desc=f"Evaluate-{split}"):
            ...
            with _autocast_context(device):
                ...
                if hasattr(model, "impute_from_regressor"):
                    reg_out = model.impute_from_regressor(
                        x_masked, lead_indices=lead_indices
                    )
                elif "y_pred" in out:
                    # AliTok exposes its primary reconstruction through the
                    # common stage-1 forward contract rather than a separate
                    # regressor method.
                    reg_out = {
                        "available": True,
                        "y_pred": out["y_pred"],
                        "z_latent": out.get("z_regressed"),
                    }
                else:
                    raise RuntimeError(
                        "Model exposes neither impute_from_regressor nor a "
                        "stage-1 y_pred reconstruction."
                    )
    ...
    metrics = {
        ...
        f"{split}/missing_mse_reg": reg_mse_total,
        f"{split}/missing_mae_reg": reg_mae_total,
        f"{split}/missing_rmse_reg": reg_rmse_total,
        f"{split}/missing_corr_reg": float(sum(reg_corr_leads) / max(len(reg_corr_leads), 1)),
    }
    if teacher_available:
...
def get_args():
    ...
    if args.model_family == "token_refiner":
        from unified_latents.engineering.experimental.token_refiner import resolve_teacher_token_length
...
def load_model(args, device):
    ...
    if args.model_family == "hybrid":
        ...
        from unified_latents.engineering.models.vae_fm import WearECGVAE
        ...
        from unified_latents.engineering.models.vae_fm import WearECGFMVAE
        ...
        from unified_latents.engineering.experimental.token_refiner import WearECGTokenRefiner
