...
def _train_variant(
    *,
    ...
    _seed(run_seed)
    template = CounterfactualMeasurementOperator(input_dim=train_x.shape[-1], classes=train_y.shape[-1]).cuda()
    ...
    for learning_rate in LEARNING_RATES:
        for weight_decay in WEIGHT_DECAYS:
            ...
            if (path / "summary.json").exists():
                continue
            model = CounterfactualMeasurementOperator(input_dim=train_x.shape[-1], classes=train_y.shape[-1]).cuda()
