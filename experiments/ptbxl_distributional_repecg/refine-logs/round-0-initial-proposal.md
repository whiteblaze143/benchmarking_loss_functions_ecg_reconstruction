...
- **Success condition:** A branch survives only if it (1) beats or stabilizes against its strongest matched representation control, (2) responds to a mechanism-destroying falsification in the predicted direction, and (3) retains diagnostic information under paired patient-level evaluation. The program succeeds scientifically if at least one new ECG object passes all three conditions, including a negative conclusion for branches that fail.
...
5. require a matched control and a causal falsification for that change.
...
PTB-XL records500
  ...
  -> matched probe and mechanism-destroying control
...
- **Model:** Operator MLP plus circular response CNN, a two-block Set Transformer, diagnosis head, and target-operator reconstruction head.
...
- **R-peak selection bias:** Report exclusion and beat counts by fold/class; add an all-record fixed-window sensitivity if fold-10 exclusion exceeds 5 percent.
...
combined with matched causal tests that can reject each proposed object. The
...
- **Required evidence:** Improvement in at least one dimension without unacceptable degradation in the other, plus phase-shuffle sensitivity.
...
- **Must-prove:** Kernel direction beyond moments; recurrence beyond means; each branch-specific mechanism is causally used.
