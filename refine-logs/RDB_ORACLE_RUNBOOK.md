...
Expected witness: one JSON object with `event=preflight_complete`, `records=2`, `selection.selected_models=31`, threshold `0.5`, manifest SHA-256 `e88f3b31bda574a73e09678f2f0aa33346a9c2c1baa31062e91a606ac9182aa2`, and no results DB creation. The evaluator requires exact checkpoint SHA-256 agreement between these LUDB rows and the archive.
...
The launcher creates detached tmux session `ecgaim_ludb_oracle_eval`. It runs LUDB with `--once`, reusing the existing SQLite database. Rows are skipped only when status, checkpoint SHA, protocol SHA, and dataset SHA all match; mismatches are recomputed under the same model ID. RDB launches only after the LUDB process exits successfully. RDB then selects only completed LUDB oracle rows with `signal_pearson_p05 >= 0.50` (currently 31/123), verifies exact archive checkpoint identities, and stores the rule and selection manifest in its protocol metadata. The RDB launcher creates detached session `ecgaim_rdb_oracle_eval`, pins it to exactly seven CPUs selected from the caller's affinity, and sets all numerical-library and Torch thread counts to seven. Both launchers refuse duplicate sessions.
