### SOURCE: https://github.com/NVlabs/NVAE/blob/master/model.py
...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, idx):
        ...
        kl_diag = []
        log_p, log_q = 0., 0.
        for i, (q, p, log_q_conv, log_p_conv) in enumerate(zip(all_q, all_p, all_log_q, all_log_p)):
            if self.with_nf:
    ...
    def batchnorm_loss(self):
        ...
        for l in self.all_bn_layers:
            if l.affine and isinstance(l, nn.SyncBatchNorm):
