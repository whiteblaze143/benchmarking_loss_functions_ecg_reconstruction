...
from learn_functions.generate_model import generate_reconstructor, generate_foundation_reconstructor, generate_optimizer
...
class ReconstructionManager(TrainingManager):
    ...
    def __init__(self,
        ...
        super().__init__(parent_folder,
                         ...
                         epochs,
                         batch_size,
                         prioritize_percent) # Fixed: super().__init__ expects prioritize_percent as 7th arg
        ...
        if model_type == 'mason':
            ...
            # Map input lead keys to indices for foundation models
            twelve_keys = get_twelve_keys()
            input_indices = [twelve_keys.index(key) for key in self.input_lead_keys]
            self.model = generate_foundation_reconstructor(model_type,
                                                             input_indices,
                                                             self.device,
                                                             target_len=self.sample_num)
    ...
    def plot_test_stats(self, plot_sub_classes = None, gamma = 0.1, loss_threshold = 0.1, time_threshold = 0.5, density = 249, compute_loss_per_sample = False):    
        ...
        if compute_loss_per_sample:
            ...
            target_indexes = intersect1d(mserrors_per_sample_target_indexes, distances_per_sample_target_indexes)
