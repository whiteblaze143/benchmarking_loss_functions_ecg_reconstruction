...
class SineDataset(Dataset):
    def __init__(
        ...
        self.pred_len, self.missing_ratio, self.synthesis_channels = (
            predict_length,
            missing_ratio,
            synthesis_channels,
        )
        print(synthesis_channels)
        ...
        if period == "test":
            if missing_ratio is not None:
                self.masking = self.imputation_mask_data(seed)
            elif predict_length is not None:
                masks = np.ones(self.samples.shape)
                masks[:, -predict_length:, :] = 0
                self.masking = masks.astype(bool)
            elif synthesis_channels is not None:
                masks = np.ones(self.samples.shape)
                masks[:, :, synthesis_channels] = 0
                self.masking = masks.astype(bool)
                if self.save2npy:
                    np.save(
                        os.path.join(self.dir, f"sine_masking_{self.window}.npy"), masks
                    )
            else:
    ...
    def sine_data_generation(
        no, seq_len, dim, save2npy=True, seed=42, dir="./", period="train"
    ):
    ...
    def imputation_mask_data(self, seed=2023):
