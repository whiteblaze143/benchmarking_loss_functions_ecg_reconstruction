...
class FiLMLayer(nn.Module):
    ...
    def forward(self, x, condition):
        film_params = self.mlp(condition)  # (bs, 2*dim)
        gamma, beta = torch.chunk(film_params, 2, dim=-1)  # (bs, dim)

        gamma = gamma.unsqueeze(1)  # (bs, 1, dim)
        beta = beta.unsqueeze(1)  # (bs, 1, dim)
...
class Transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        self.ecg_emb = Conv_MLP(n_feat, n_embd, resid_pdrop=resid_pdrop)
        ...
        self.lead_enc = LeadEmbedding(lead_num=12)
        ...
        self.ecg_text_fusion_fc = nn.Linear(2 * n_embd, n_embd)
        self.film = FiLMLayer(n_embd)
        self.use_text = use_text

    def forward(self, x, t, padding_masks=None, return_res=False, text_embed=None):
        x = self.lead_enc(x)
        ecg_emb = self.ecg_emb(x)
        inp_enc = self.pos_enc(ecg_emb)
        enc_emb = self.encoder(inp_enc, t, padding_masks=padding_masks)

        if self.use_text:
            text_emb_expanded = text_embed.unsqueeze(1).expand(-1, 1000, -1)
            gate = torch.sigmoid(
                self.ecg_text_fusion_fc(torch.cat([enc_emb, text_emb_expanded], dim=-1))
            )
            cond_emb = gate * enc_emb + (1 - gate) * text_emb_expanded
        else:
            cond_emb = enc_emb

        inp_dec = self.pos_dec(ecg_emb)
        output, mean, trend, season = self.decoder(
            inp_dec, t, cond_emb, padding_masks=padding_masks
        )
