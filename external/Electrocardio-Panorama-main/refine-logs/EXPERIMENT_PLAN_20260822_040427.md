...
- **Exact-baseline distinction**: `checkpoints/factorial_ecg_aim_1010010_s42.metadata.json` records `observed_leads=[0,1,7]` (I, II, V2), and no matching one-lead `1010010`, seed-42 artifact was found. Valid one-lead checkpoints therefore exist and should anchor compatibility/evaluator checks, but the exact A0 required here—one observed lead, mask `1010010`, seed 42—still requires a controlled run unless a matching artifact is subsequently located.
...
Use **Lead II (index 1)** for the fixed-source experiment: it is already established in the one-lead queue/checkpoint family and matches Panorama's one-lead convention. Use the Lead-II one-lead checkpoint for refactor/evaluator compatibility, but train the `1010010`, seed-42 A0 from scratch for the headline comparison because its loss mask and seed differ.
...
- **Why this block exists**: the supplied baseline identity conflicts with checkpoint metadata.
...
|---|---|---|---|---|---|
| M0 | Freeze provenance and evaluator | R001–R003 | Stop unless the Lead-II one-lead checkpoint loads/reproduces, the 3-lead reference is labeled correctly, hashes freeze, and learned refactor is numerically equivalent | CPU + <1 A100-hour; 0.5–1 day | Stale queue status/mixed task labels; trust embedded checkpoint provenance and keep exact protocols separate |
...
| M2 | Establish baseline and angular screen | R006–R010 | A0 converges; C1/D1/E1 remain ≥0.54 Pearson p05 or provide a clear diagnostic | ~5 full trainings; 1–3 days | Wasted screen compute; use staged validation and stop clearly divergent runs |
...
- **Must-run training count**: 19 full trainings if F1 is available (R006–R025 excluding metadata/evaluation-only runs and with seed-42 reuse); 16 if F1 is blocked and E1 is the candidate. G1 and withheld-source testing are additional.
...
- **Biggest bottleneck**: obtaining and validating a licensed canonical lead-field asset. A smaller setup cost is training the exact Lead-II `1010010`, seed-42 A0; existing Lead-I/II seed-201 checkpoints already validate the one-lead path.
...
- **Task-label leakage / source mismatch**: both one-lead and 3-lead checkpoint families exist, while queue status is stale. Mitigation: read embedded provenance, use Lead II, hash every artifact, and train the exact one-lead `1010010`, seed-42 A0.
...
- [x] Existing one-lead checkpoints are recognized and exact-mask/seed differences are explicit
