...
def _train_variant(
    *,
    ...
    _seed(run_seed)
    template = RecurrenceCNN(classes=train_y.shape[-1], variant=variant_obj).cuda()
    ...
    for learning_rate in LEARNING_RATES:
        for weight_decay in WEIGHT_DECAYS:
            ...
            if (path / "summary.json").exists():
                continue
            model = RecurrenceCNN(classes=train_y.shape[-1], variant=variant_obj).cuda()
...
def main() -> None:
    ...
    variant_registry = get_variants_for_paper(1)
    ...
    if rep_key not in train or rep_key not in validation:
        raise ValueError(f"Paper 1 representation artifact lacks {rep_key}")
