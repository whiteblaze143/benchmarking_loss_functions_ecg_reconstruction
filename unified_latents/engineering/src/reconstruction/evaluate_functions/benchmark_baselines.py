...
project_root = "/home/mithunmanivannan/projects/benchmarking_loss_functions_ecg_reconstruction/unified_latents/engineering"
sys.path.append(project_root)
sys.path.append(os.path.join(project_root, "third_party/cNVAE_ECG/conditional"))
sys.path.append(os.path.join(project_root, "third_party/ecg_reconstruction"))

try:
    from src.data.multi_source_dataset import MultiSourceECGDataset
    from src.training.train_mason import MasonReconstructor
except ImportError:
    MultiSourceECGDataset = None
    MasonReconstructor = None
from learn_functions.legacy_mason import LegacyMasonReconstructor
...
class CNVAEWrapper(nn.Module):
    ...
    def forward(self, x, metadata):
        ...
        if recon_small.shape[1] == 3:
            ...
            recon[:, 3, :] = recon[:, 0, :] - recon[:, 1, :]/2   # aVL (Mason index 3)
            recon[:, 4, :] = -(recon[:, 0, :] + recon[:, 1, :])/2   # aVR (Mason index 4)
