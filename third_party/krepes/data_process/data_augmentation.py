...
class AugmentedDataset(torch.utils.data.Dataset):
    def __init__(self, original_data, augmenter):
        self.features = torch.tensor(original_data.iloc[:, :-1].values, dtype=torch.float32)
        self.targets = torch.tensor(original_data.iloc[:, -1].values, dtype=torch.long)
...
#     def __init__(self, X: np.ndarray, augmenter: ScarfAugmenter, y: Optional[np.ndarray] = None, device: str = "cuda"):
...
#     def __getitem__(self, idx):
#         x = self.X[idx]
#         x1 = self.aug(x.unsqueeze(0)).squeeze(0)
#         x2 = self.aug(x.unsqueeze(0)).squeeze(0)
#         if self.y is None:
#             return x1, x2
#         else:
