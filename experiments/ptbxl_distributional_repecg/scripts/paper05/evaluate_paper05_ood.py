...
VARIANTS = list(get_variants_for_paper(5).keys())
...
def main() -> None:
    ...
    for file in sorted(args.representations.glob("representation_ood_*.npz")):
        ...
        for variant in VARIANTS:
            ...
            variant_obj = get_variants_for_paper(5)[variant]
            ...
            model = KoopmanOperatorModel(input_dim=x_val.shape[-1], classes=5, variant=get_variants_for_paper(5)[variant]).cuda()
