#!/usr/bin/env python3
"""Evaluate temporal micro-structure (delineation) on LUDB.
...
def get_gt_delineations(record_path: Path):
    """Parse GT boundaries from PhysioNet LUDB .atr files for all 12 leads."""
    gt_bounds = {}
    
    for lead in LEADS:
        ext = lead
        try:
            ann = wfdb.rdann(str(record_path), extension=ext)
            ...
            # Simple state machine to extract boundaries based on peak type
            ...
            # Missing lead or missing annotation
            gt_bounds[lead] = {
                'P_Onset': np.array([]),
                'P_Offset': np.array([]),
                'R_Onset': np.array([]),
...
def compute_delineation_metrics(pred_bounds, gt_bounds, tolerance=75): # 75 samples = 150ms tolerance
...
def main():
    ...
    # model_ids = ["unet__e0c0m0d0__s42"] # baseline testing
    
    data_dir = project_root / "data/ludb"
    record_ids = [p.stem for p in data_dir.glob("*.dat")]
    record_ids = sorted(list(set(record_ids)), key=int)
    logging.info(f"Evaluating {len(model_ids)} models on {len(record_ids)} LUDB records.")
    ...
    for rid in record_ids:
        ...
        try:
            ...
            gt_bounds = get_gt_delineations(rec_path)
            
            # Reorder channels to match LEADS exactly
            signal_np = record.p_signal # (5000, 12)
            channels = record.sig_name
            # Typically LUDB channels are lower case like LEADS, we map them
            ordered_signal = np.zeros((len(LEADS), 5000))
            ...
            logging.error(f"Failed to load LUDB {rid}: {e}")
            continue
            
    out_dir = project_root / "results/factorial_v4"
    out_dir.mkdir(parents=True, exist_ok=True)
    summary_file = out_dir / "ludb_delineation_evaluation.csv"
    ...
    for model_id in model_ids:
        ...
        recon_nps = []
        batch_size = 64
        for i in tqdm(range(0, len(ludb_data), batch_size), desc=f"GPU Inference {model_id}"):
            batch_data = ludb_data[i:i+batch_size]
