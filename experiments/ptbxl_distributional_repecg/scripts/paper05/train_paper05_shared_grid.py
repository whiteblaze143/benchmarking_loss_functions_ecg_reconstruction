...
def _train_variant(
    *,
    ...
    _seed(run_seed)
    template = KoopmanOperatorModel(input_dim=train_x.shape[-1], classes=train_y.shape[-1], variant=variant_obj).cuda()
    ...
    for learning_rate in LEARNING_RATES:
        for weight_decay in WEIGHT_DECAYS:
            ...
            if (path / "summary.json").exists():
                continue
            model = KoopmanOperatorModel(input_dim=train_x.shape[-1], classes=train_y.shape[-1], variant=variant_obj).cuda()
...
def main() -> None:
    ...
    variant_registry = get_variants_for_paper(5)
    ...
    if rep_key not in train:
        raise ValueError(f"Paper 5 representation {rep_key!r} is absent; available={sorted(train)}")
