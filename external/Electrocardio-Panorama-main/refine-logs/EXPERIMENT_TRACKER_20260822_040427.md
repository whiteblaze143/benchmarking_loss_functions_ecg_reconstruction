...
|---|---|---|---|---|---|---|---|---|
| R001 | M0 | verify existing one-lead baseline path | `1lead_ecg_aim_f_1000000_s201_l1.pt` (Lead II) plus checkpoint inventory | PTB-XL validation/evaluator | embedded provenance, loadability, hashes, metrics | MUST | TODO | Checkpoint exists although queue says pending; mask/seed are compatibility-only |
| R002 | M0 | freeze exact headline protocol | Lead II, mask `1010010`, seed 42 config audit | PTB-XL train/val/test | split hashes, loss/config diff | MUST | TODO | Existing Lead-II checkpoints establish source and path; exact mask/seed A0 remains a new controlled run |
| R003 | M0 | prove refactor parity | archived active implementation vs centralized learned mode loaded from Lead-II checkpoint | deterministic tiny batch | outputs, gradients, state keys | MUST | TODO | FP32 `atol=1e-6, rtol=1e-5`; do not edit backup file |
...
1. **R001** — load and evaluate the existing Lead-II one-lead checkpoint, recording embedded provenance and correcting for stale queue status.
2. **R002** — freeze the Lead-II, `1010010`, seed-42 headline config and split/protocol hashes.
...
| 2026-08-22 | Exact one-lead `1010010`, seed-42 artifact was not found | one-lead inventory covers seed 201 and four other masks; named `1010010_s42` metadata is 3-lead | train exact A0 from scratch while retaining existing one-lead checkpoint as compatibility anchor |
