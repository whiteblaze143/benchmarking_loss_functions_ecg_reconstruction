...
class Net1D(nn.Module):
    ...
    def forward(self, x):
        ...
        out = x
        stage_outputs = []  # 初始化一个列表来存储每个stage的输出
        ...
        for i_stage in range(self.n_stages):
            ...
            out = net(out)
            stage_outputs.append(out) # 存储当前stage的输出
...
def ft_ECGFounder(device, n_classes=1000):
    model = Net1D(
        in_channels=12,
        ...
        filter_list=[
            ...
            1024,
            1024,
        ],  # [16,32,32,80,80,256,256] [32,64,64,160,160,512,512] [64,160,160,400,400,1024,1024]
    ...
    checkpoint = torch.load(
        "evaluation/weight/ECGFounder_model.pth",
        map_location=device,
    )
    ...
    model.dense = nn.Identity()  # get representation
