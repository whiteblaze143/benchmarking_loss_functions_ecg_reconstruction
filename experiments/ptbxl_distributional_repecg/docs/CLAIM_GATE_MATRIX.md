...
| 9 | Counterfactual regularizers add stability under intervention | standard continuous measurement-operator ECG | randomized counterfactual intervention angle | original-to-sham prediction stability minus original-to-destroyer stability | task AUROC drop | counterfactual state offers no material stability |
| 10 | Interventional stability outperforms observational representation | observational repStat | random non-causal perturbation (sham intervention) | out-of-distribution paired set stability improvement | task AUROC drop | interventional stability provides no robustness |
| 11 | Future predictive states capture causal dynamics | past-history predictive state | chronological link breaking | future RKHS embedding prediction error | predictive state collapse | states collapse to simple occupancy |
| 12 | Causal ordering isolates structural innovations | raw residual prediction | innovation ordering destruction | ordered minus unordered predictive AUROC | residual reconstruction error | innovations are indistinguishable from unstructured noise |
| 13 | Distributional surgery isolates causal features | standard feature masking | mismatched/random surgery destruction | aligned minus mismatched surgery representation similarity | task AUROC drop | surgery yields no robustness improvement |
| 14 | IRM applied to interventional domains improves generalization | ERM pooling across domains | domain assignment shuffle | out-of-domain IRM AUROC minus ERM AUROC | task AUROC drop in-domain | IRM provides no out-of-domain generalization benefit |
| 15 | Factorized mechanisms are statistically independent | entangled representation (standard VAE) | random basis mixing (entanglement) | factorized minus entangled downstream causal query accuracy | independent mechanism statistical dependence | components fail to exhibit causal independence |
