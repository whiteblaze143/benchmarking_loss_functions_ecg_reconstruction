...
- Resolve available Kingston channel names explicitly; never infer them by column position.
- Map available channels into `[I, II, V1, V2, V3, V4, V5, V6]`.
...
- Kingston diagnostic metrics are reported only if independently mapped labels contain both classes for that endpoint. Otherwise store them as `ineligible`; representation/prediction-shift endpoints remain valid.
