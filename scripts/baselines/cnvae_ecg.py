...
class CNVAE_CardiacStateDecoder(nn.Module):
    ...
    def forward(self, fm_tokens, use_posterior=None, deterministic=False):
        ...
        for s_idx in range(self.num_latent_scales):
            ...
            latent_dist = q_dist if use_posterior else p_dist
            z = latent_dist.mu if deterministic else latent_dist.sample()
            all_z.append(z)
    ...
    def forward(self, fm_tokens, use_posterior=None, kl_balance=False, deterministic=False):
        if use_posterior is None:
            use_posterior = self.training
    ...
    def _build_12lead(self, x_input: torch.Tensor, precordial: torch.Tensor,
        ...
        Any two observed Einthoven limb leads determine the third and augmented
        limb leads. Observed precordial leads overwrite decoder outputs so the
        returned signal preserves the acquisition exactly.
        ...
        if x_input.shape[-1] != T:
            x_input = F.interpolate(x_input, size=T, mode="linear", align_corners=False)

        values = {
            index: torch.zeros(B, T, device=device, dtype=precordial.dtype)
            for index in (0, 1, 2)
        }
        present = {
            index: (li_2d == index).any(dim=1)
            for index in (0, 1, 2)
        }
        batch_indices = torch.arange(B, device=device)
        for index in (0, 1, 2):
            valid = batch_indices[present[index]]
            if valid.numel():
                positions = (li_2d[valid] == index).long().argmax(dim=1)
                values[index][valid] = x_input[valid, positions, :]

        have_i_ii = present[0] & present[1]
        have_i_iii = present[0] & present[2]
        have_ii_iii = present[1] & present[2]
        values[2][have_i_ii] = values[1][have_i_ii] - values[0][have_i_ii]
        values[1][have_i_iii] = values[0][have_i_iii] + values[2][have_i_iii]
        values[0][have_ii_iii] = values[1][have_ii_iii] - values[2][have_ii_iii]
        derivable = have_i_ii | have_i_iii | have_ii_iii
        if derivable.any():
            out[derivable, 0, :] = values[0][derivable]
            out[derivable, 1, :] = values[1][derivable]
            out[derivable, 2, :] = values[2][derivable]
            out[derivable, 3, :] = -(
                values[0][derivable] + values[1][derivable]
            ) / 2.0
            out[derivable, 4, :] = (
                values[0][derivable] - values[2][derivable]
            ) / 2.0
            out[derivable, 5, :] = (
                values[1][derivable] + values[2][derivable]
            ) / 2.0

        # Preserve every observed lead exactly, including observed chest leads.
        for position in range(li_2d.shape[1]):
            out[batch_indices, li_2d[:, position], :] = x_input[:, position, :]
        return out
...
class ECGConvECGFMBridge(nn.Module):
    ...
    def forward(self, x, lead_indices=None, kl_balance=False, **kwargs):
        ...
        dec_out = self.decoder(
            fm_tokens,
            use_posterior=kwargs.get('use_posterior'),
            kl_balance=kl_balance,
            deterministic=kwargs.get('deterministic', not self.training),
        )
