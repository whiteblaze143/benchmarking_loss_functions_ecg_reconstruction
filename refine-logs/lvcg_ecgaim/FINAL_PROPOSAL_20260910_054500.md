...
- Reused unchanged: current ECG-AIM reconstruction model; LVCG `BeatEncoder`; LVCG `GlobalRREmbedding`.
...
Pilot training leaves the ECG-AIM reconstruction path unchanged and trains the auxiliary branch with downstream linear-probe supervision only in a separate training stage. This prevents a reconstruction change from being mistaken for an embedding improvement. If the representation pilot passes, a later experiment may jointly train with a small auxiliary weight, but that is outside the initial claim.
